...
export default function ActTable() {
  ...
  const limpar = () => { setBusca(''); setTipo('todos'); setOrgao('todos'); setAno('todos'); setStatus('todos'); setNome(''); setSiape(''); setProcesso(''); setSoRel(false); setSoSei(false); };
  const temFiltro = busca || tipo !== 'todos' || orgao !== 'todos' || ano !== 'todos' || status !== 'todos' || nome || siape || processo || soRel || soSei;
      ...
      <div className="bg-white p-3 rounded-lg border border-slate-200 shadow-xs space-y-2.5">
        <div className="relative">
          ...
          <input value={busca} onChange={e => setBusca(e.target.value)}
            placeholder="Buscar por número, ementa, processo…"
        ...
        <div className="flex items-center gap-2.5 flex-wrap bg-slate-50 p-2 rounded-md border border-slate-100 text-xs">
          ...
          <input value={processo} onChange={e => setProcesso(e.target.value)} placeholder="Processo…"
            title="Todos os atos que citam este processo. Aceita com ou sem pontuação, e aceita só um pedaço do número."
            className="w-44 bg-white border border-slate-200 rounded px-2 py-1 text-xs font-mono" />
          ...
          <table className="w-full text-left border-collapse min-w-[900px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200 text-[11px] font-bold text-slate-500 uppercase">
                ...
                <th className="py-2 px-2.5 w-44" title="Número do processo administrativo. Antes do SEI (setembro/2017) o processo era em papel, com o mesmo formato de número.">Processo</th>
