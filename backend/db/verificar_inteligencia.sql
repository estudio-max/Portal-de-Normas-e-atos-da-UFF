...
--
--  ORDEM DOS BLOCOS, e por que ela não é arbitrária: os blocos 1 a 9 tocam só
--  as tabelas do projeto, sem qualificar o banco; os blocos 10 a 13 consultam
--  `information_schema`. Eles vêm por último de propósito. Na mesma sessão de
--  importação de 03/08/2026, com `SELECT DATABASE()` devolvendo
--  `fanara87_governanca` corretamente, o bloco que vinha depois de um
--  `FROM information_schema.TABLES` estourou com "#1109 Tabela
--  'evidencia_fato' desconhecida em 'information_schema'": o phpMyAdmin
--  re-deriva o banco corrente ao analisar cada consulta, e a referência
--  explícita a `information_schema` vira o contexto para o que vem depois.
--  Com as frágeis na frente, o arquivo sobrevive às duas abas.
--
--  Cada bloco traz o resultado ESPERADO ao lado. Os blocos 3 a 9 são as TRAVAS
--  DE PUBLICAÇÃO — enquanto qualquer um deles devolver linha, nada daquele
--  módulo vai ao ar.
--
...
--    ESPERADO: `fanara87_governanca` (ou o banco da UFF, depois do cutover).
--    Se vier `information_schema` ou NULL, pare: metade dos blocos abaixo usa
--    ZERO linhas como sinal de saúde, e um banco errado devolve zero em todos
--    eles por não achar tabela nenhuma — o arquivo inteiro passa a mentir.
SELECT DATABASE() AS banco_atual, VERSION() AS versao;

-- 1) CONTAGEM POR TABELA (linha de base do backfill) -------------------------
--     Logo após a migração: doze linhas, todas ZERO.
...
-- 2) EVIDÊNCIA ÓRFÃ (o preço da referência polimórfica) ----------------------
...
-- 3) TRAVA: NADA PÚBLICO SEM FONTE E TRECHO ----------------------------------
...
-- 4) TRAVA: CONFIANÇA BAIXA NÃO VAI AO FEED ----------------------------------
...
-- 5) TRAVA: INDICADOR DENTRO DE 0–100 E COERENTE -----------------------------
...
-- 6) TRAVA: PRAZO DECLARADO SEM DATA, E VICE-VERSA ---------------------------
...
-- 7) O REGISTRO DE COMISSÕES NÃO DIVERGIU ------------------------------------
...
-- 9) TRAVA: OBRIGAÇÃO SEM ATO DE ORIGEM VISÍVEL ------------------------------
--    A FK garante que o ato existe; isto garante que ele está PUBLICÁVEL — sem
--    uid não há link para a ficha, e item sem link vira afirmação sem fonte.
--    ESPERADO: ZERO linhas.
SELECT o.id, o.uid
  FROM obrigacao o JOIN ato a ON a.id = o.ato_origem_id
 WHERE a.uid IS NULL OR TRIM(a.uid) = ''
 LIMIT 50;

-- ===========================================================================
--  A PARTIR DAQUI: blocos que consultam `information_schema`.
--  Ficam no fim porque a referência explícita ao `information_schema` vira o
--  banco corrente do phpMyAdmin para os statements seguintes (ver cabeçalho).
-- ===========================================================================

-- 10) AS DOZE TABELAS EXISTEM ------------------------------------------------
...
-- 12) TODA TABELA TEM CHAVE NATURAL ------------------------------------------
...
-- 13) AS CHAVES ESTRANGEIRAS ENTRARAM ----------------------------------------
