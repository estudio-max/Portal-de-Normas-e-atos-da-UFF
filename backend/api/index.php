...
// ---- COBERTURA DO BOLETIM -------------------------------------------------
// O painel de mandatos afirma "o setor X está sem chefia formalmente
// constituída desde D". A afirmação só se sustenta se a base cobriu o BS
// INTEIRO de D até hoje: da beirada de dentro, um ano mal carregado é
// indistinguível de um ano em que ninguém foi designado. Sem este guarda o
// painel acusaria de acefalia setores que só estão mal indexados — e o
// gabinete descobriria isso na primeira conferência.
//
// O esperado não é chute: a numeração do BS é sequencial dentro do ano, então
// o MAIOR número do ano diz quantos boletins existiram. Isso se auto-calibra
// (2025 fechou em 153, 2023 em 242) e não depende de constante mágica.
//
// Ponto cego conhecido: se a base tivesse só o COMEÇO de um ano (boletins
// 1..16 de 245), o maior seria 16 e a cobertura pareceria 100%. Nas cargas
// parciais reais isso não acontece — elas pegaram o FIM do ano (2022 tem os
// boletins 224..245, 2019 tem 223..238) — mas, como é sorte e não garantia,
// um ano fechado cujo maior número seja implausível (< 100, quando a série
// histórica fica entre ~150 e ~245) é marcado 'indeterminada': a base não
// sabe nem quantos existiram, então não pode afirmar nada sobre o ano.
function cobertura_por_ano(PDO $pdo): array {
    $rows = $pdo->query("
        SELECT ano, COUNT(*) AS carregados, MAX(CAST(numero AS UNSIGNED)) AS ultimo
        FROM boletins WHERE ano IS NOT NULL AND ano > 0
    ...
    foreach ($rows as $r) {
        ...
        $ultimo = (int)$r['ultimo'];
        // Ano corrente ainda está sendo publicado: o último número é o de
        // agora, não o do fim do ano — nunca marcar de indeterminado por isso.
        $indeterminada = ($ano < $anoAtual && $ultimo < 100);
        $cob[$ano] = [
            'ano' => $ano,
            'carregados' => $carregados,
            'publicados' => $ultimo,
            'pct' => $ultimo > 0 ? round(100 * $carregados / $ultimo) : 0,
            'confiavel' => !$indeterminada && $ultimo > 0 && $carregados >= 0.9 * $ultimo,
        ];
    }
    return $cob;
}

// A janela [desde, hoje] é confiável se TODO ano dela é confiável. É a
// pergunta que o painel precisa responder antes de afirmar acefalia.
function janela_coberta(array $cob, string $desde): bool {
    $ini = (int)substr($desde, 0, 4);
    for ($a = $ini; $a <= (int)date('Y'); $a++) {
...
// a pureza é de 100% (Chefe 898/898 em 2 anos; Coordenador 897/897 em 4), e em
// 128 mil atos NÃO existe um único mandato que não seja 2 ou 4 anos.
// Pró-Reitor/Superintendente/Gerente ficam de fora de propósito: 181
// designações, ZERO com prazo — servem a gestão, não a mandato fixo.
//
// A regra mora AQUI, na projeção, e não no extrator: ato_funcoes registra o
// que o ato disse. Gravar 24 meses num ato que não declarou nada apagaria para
// sempre a diferença entre o que é lei e o que é dedução — e o reprocessamento
// passaria a reescrever a dedução como se fosse fato.
const REGRA_MANDATO = [
...
function mandatos(PDO $pdo): void {
    ...
    if (!$existe) { responder_json(['total' => 0, 'atualizadoEm' => date('Y-m-d'), 'setores' => [], 'cobertura' => []]); }
    ...
    // (unidade_chave, cargo). Sem o corte de 4 anos — ele existe lá para não
    // mostrar titular fantasma, mas aqui a posição vencida é exatamente o que
    // se procura; descartá-la esconderia o achado.
    $rows = $pdo->query("
        SELECT f.cargo, f.unidade, f.unidade_chave, f.siape,
               COALESCE(NULLIF(f.nome,''), s.nome) AS nome,
    ...
    // Situação atual da PESSOA (mesma regra da aba Chefias): só permanece se o
    // último evento dela for exatamente esta designação. Resolve unidade
    // renomeada e pessoa que mudou de cargo.
    $ult = [];
    $ev = $pdo->query("
        SELECT f.siape, f.acao, f.unidade_chave, LOWER(f.cargo) AS cargo, a.data_ato
        FROM ato_funcoes f JOIN atos a ON a.id = f.ato_id
        WHERE f.siape <> '' AND a.data_ato IS NOT NULL");
    while ($e = $ev->fetch()) {
        $s = $e['siape'];
        $m = $ult[$s] ?? null;
    ...
    foreach ($cand as $c) {
        ...
        $cargoLow = mb_strtolower(trim($c['cargo']));
        $declarado = (int)($c['prazo_meses'] ?? 0);
        $prazo = $declarado ?: (REGRA_MANDATO[$cargoLow] ?? 0);
        if (!$prazo) continue;
        ...
        if ($fim >= $hoje) {
            $situacao = 'em_dia';
        } elseif (!janela_coberta($cob, $fim)) {
            // Da beirada de dentro, ano mal carregado é indistinguível de ano
            // sem designação: não dá para afirmar acefalia aqui.
            $situacao = 'sem_cobertura';
        } else {
            $situacao = 'sem_chefia';
        }
        ...
        $setores[] = [
            ...
            'prazoMeses' => $prazo,
            // O gabinete tem que ver, na linha, se o prazo é o que o ato disse
            // ou o que a regra do cargo deduziu. Sem isto ele lê uma data e não
            // sabe se é lei ou dedução.
